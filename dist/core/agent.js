...
import { saveToManifest } from "../utils/saveManifest";
dotenv.config();
const client = new OpenAI({
    apiKey: process.env.OPENROUTER_API_KEY,
    baseURL: "https://openrouter.ai/api/v1",
});
function createToonPayload(filePath, fileContent) {
    return `
file "${filePath}" {
    content: """
${fileContent}
...
async function main() {
    ...
    const response = await client.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You are an AI agent analyzing a source file in TOON format. Return ONLY JSON with keys: updated_code (full improved code), summary, next_steps. Keep functionality intact."
            },
